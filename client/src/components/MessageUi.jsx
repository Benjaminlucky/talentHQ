...
export default function MessagesUI() {
  ...
  useEffect(() => {
    ...
    import("socket.io-client").then(({ io }) => {
      ...
      socket.on("connect", () => {
        ...
        if (current) socket.emit("join_conversation", current._id);
      });
      ...
      socket.on(
        "new_message",
        ({ message, conversationId, conversationUpdate }) => {
          const current = activeConvoRef.current;
          ...
          setConversations((prev) =>
            prev.map((c) => {
              ...
              const isOpen =
                current && current._id.toString() === conversationId.toString();
              return {
                ...c,
      ...
      socket.on("conversation_updated", (update) => {
        ...
        setConversations((prev) =>
          prev.map((c) => {
            if (c._id.toString() !== update._id.toString()) return c;
            const isOpen =
              current && current._id.toString() === update._id.toString();
            return {
              ...c,
              lastMessage: update.lastMessage,
              lastMessageAt: update.lastMessageAt,
              unread: isOpen ? 0 : update.unread,
            };
          }),
        );
      });
  ...
  useEffect(() => {
    ...
    if (!socket) return;
    if (activeConvo) socket.emit("join_conversation", activeConvo._id);
    return () => {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(() => {
      ...
      if (activeConvoRef.current)
        loadMessages(activeConvoRef.current._id, true);
    }, 5000);
    return () => {
      if (pollRef.current) clearInterval(pollRef.current);
    };
  }, [socketConnected, loadConversations, loadMessages]);
        ...
        >
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                title={socketConnected ? "Live" : "Polling"}
