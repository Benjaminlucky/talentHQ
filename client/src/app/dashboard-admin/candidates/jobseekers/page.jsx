...
const PLANS = ["jobseeker_free", "jobseeker_pro", "jobseeker_premium"];
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={form.activePlan || "jobseeker_free"}
...
export default function JobseekersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      notify("Failed to load jobseekers", "error");
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (user) => {
    ...
    try {
      ...
      notify("Failed to load jobseeker details", "error");
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const {
        ...
        oauthProvider, oauthId, skill, certifications,
        workExperience, education, projects, ...updates
      } = editForm;
      await axios.patch(
        `${API}/api/superadmin/users/${editUser._id}`,
        { role: "jobseeker", ...updates },
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        },
      ...
      setEditUser(null);
      notify("Jobseeker updated successfully");
      fetchUsers();
    } catch (err) {
      notify(
        err.response?.data?.message || "Failed to update jobseeker",
        "error",
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notify("Jobseeker deleted successfully");
      fetchUsers();
    } catch (err) {
      notify(
        err.response?.data?.message || "Failed to delete jobseeker",
        "error",
      );
    } finally {
      setDeleting(false);
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <UserCheck size={22} className="text-primary-500" /> Jobseekers
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {total} jobseeker{total !== 1 ? "s" : ""} registered on the
            platform
          </p>
        </div>
        <Link href="/dashboard-admin/candidates/add-jobseeker">
