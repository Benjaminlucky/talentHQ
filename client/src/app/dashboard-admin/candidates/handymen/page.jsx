...
const TRADES = [
  "Plumber","Electrician","Carpenter","Painter","Mason / Bricklayer",
  "Welder / Fabricator","Tiler / Floor Layer","Glazier","Roofer",
  "AC Technician","Generator Technician","Mechanic","Vulcanizer",
  "Barber / Hair Stylist","Tailor / Fashion Designer","Photographer",
  "Caterer / Chef","Cleaner","Security Guard","Landscaper / Gardener","Other",
];
const PLANS = ["handyman_free", "handyman_pro", "handyman_premium"];
...
function PlanBadge({ plan }) {
  ...
  return (
    ...
    >
      {free
        ? "Free"
        : plan.replace("handyman_", "").replace(/^\w/, (c) => c.toUpperCase())}
    </span>
  );
}
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-5">
          <span className="font-semibold">{user.fullName}</span> will be
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("yearsExperience", Number(e.target.value))
                  }
                  className={INP}
                />
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Short bio about this handyman..."
                  className={`${INP} resize-none`}
                />
              </div>
            </div>

            <SectionHeading label="Skills & Certifications" />
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Pipe fitting, Drainage repair, Installation"
                  className={INP}
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={form.activePlan || "handyman_free"}
...
export default function HandymenPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/api/superadmin/users`, {
        params: { role: "handyman", search: debouncedSearch, page, limit: 20 },
      ...
      notify("Failed to load handymen", "error");
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = async (user) => {
    ...
    try {
      ...
      notify("Failed to load handyman details", "error");
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditUser(null);
      notify("Handyman updated successfully");
      fetchUsers();
    } catch (err) {
      notify(
        err.response?.data?.message || "Failed to update handyman",
        "error",
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notify("Handyman deleted successfully");
      fetchUsers();
    } catch (err) {
      notify(
        err.response?.data?.message || "Failed to delete handyman",
        "error",
      );
    } finally {
      setDeleting(false);
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <Wrench size={22} className="text-primary-500" /> Handymen
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {total} handyman{total !== 1 ? "s" : ""} registered on the platform
          </p>
        </div>
        <Link href="/dashboard-admin/candidates/add-handyman">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="font-bold text-gray-800 mb-1">No handymen found</h3>
