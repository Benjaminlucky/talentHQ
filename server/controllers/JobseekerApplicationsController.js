...
import { toDownloadUrl } from "../utils/cloudinaryUrl.js";
...
export const getResume = async (req, res) => {
  try {
    ...
    // Build a friendly download name from the candidate, e.g. "Adaeze_Nwosu_Resume".
    const name = app.jobseeker?.fullName
      ? `${app.jobseeker.fullName}_Resume`
      : "Resume";

    res.json({
      resume: app.resume, // original URL (inline view)
      download: toDownloadUrl(app.resume, name), // forced download with extension
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
